...
type AttributeUnsafeChars struct{}

// ReviewerName returns the reviewer name.
func (a AttributeUnsafeChars) ReviewerName() string {
	return "attribute-unsafe-chars"
}

// Accepts checks if the file can be reviewed.
func (a AttributeUnsafeChars) Accepts(path string) bool {
	return true
}

// Review return a fault for each attribute value that contains an unsage character.
func (a AttributeUnsafeChars) Review(path string, page io.Reader) ([]Fault, error) {
	var fault []Fault
